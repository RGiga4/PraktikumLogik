...
print_truthtable fm =
  let ats = atoms fm
   in let width =
            ...
            ((length .
              pname .
              (maximumBy (\(P s) (P t) -> compare (length s) (length t))))
               ats)
       in let fixw s = s ++ (take (width - (length s)) (repeat ' '))
           in let truthstring p =
                    fixw
                      (if p
                         then "true"
                         else "false")
               in let mk_row v =
                        let lis = map (\x -> truthstring (v x)) ats
                         ...
                         in putStrLn ((unwords lis) ++ "| " ++ ans)
                   in let separator =
                            take (width * (length ats) + 9) (repeat '-')
                       in let printer v ats1 =
                                case ats1 of
                                  [] -> mk_row v
                                  (p:ps) ->
                                    let v' t q =
                                     ...
                                     in (printer (v' False) ps) >>
                                        (printer (v' True) ps)
                           in do putStrLn
                                   ((concatMap (\(P s) -> fixw s) ats) ++
                                    "| formula")
                                 putStrLn separator
                                 printer (\x -> False) ats
